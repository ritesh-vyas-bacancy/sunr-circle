...
class OpenComplaintsScreen extends ConsumerWidget {
  const OpenComplaintsScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(openComplaintsProvider);

    return AppScaffold(
      title: context.tr('nav.open_complaints'),
      showBack: true,
      body: RefreshIndicator(
        onRefresh: () => ref.read(openComplaintsProvider.notifier).refresh(),
            ...
            : state.error != null
                ...
                : state.complaints.isEmpty
                    ? EmptyStateView(
                        ...
                        title: context.tr('complaint.messages.no_open'),
                      )
                    : ListView.builder(
                        ...
                        itemBuilder: (context, i) {
                          ...
                          return ComplaintActionCard(
                            ...
                            onAccept: () async {
                              final notifier = ref
                                  .read(complaintDetailProvider(c.id).notifier);
                              await notifier.load(c.id);
                              ...
                              if (ok && context.mounted) {
                                ...
                                ref
                                    .read(openComplaintsProvider.notifier)
                                    .refresh();
                              }
                            },
                          );
